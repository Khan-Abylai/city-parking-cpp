...
GstreamerReader::GstreamerReader(string ip,
                                 shared_ptr<SharedQueue<shared_ptr<FrameData>>> frameQueue)
        : ILogger("Camera " + ip),
          cameraIp{std::move(ip)}, frameQueue{std::move(frameQueue)} {
...
void GstreamerReader::createStreamDecodingPipeline() {
    ...
    string decode ="avdec_h264 ! gldownload";
...
GstFlowReturn GstreamerReader::newDecodedSample(GstAppSink *appSink, gpointer data) {
    ...
    if (!currentFrame.empty())
        thisPointer->frameQueue->push(make_shared<FrameData>(thisPointer->cameraIp, std::move(currentFrame.clone()),
